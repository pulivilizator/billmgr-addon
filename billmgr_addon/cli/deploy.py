...
from ..utils.files import create_plugin_symlinks, get_standard_paths, get_mgr_paths
...
def install(plugin_name, force, xml_path):
    ...
    try:
        ...
        reload_result = subprocess.run(['systemctl', 'reload', 'billmgr'], capture_output=True, text=True)
        if reload_result.returncode != 0:
            click.echo(f"Предупреждение: не удалось перезагрузить BILLmanager: {reload_result.stderr}")
...
def remote_deploy(environment, plugin_name, config, backup, install, restart_billmgr, dry_run, xml_path):
    ...
    try:
        ...
        if missing_fields:
            raise click.ClickException(f"Отсутствуют обязательные поля в конфигурации: {missing_fields}")
        ...
        if backup:
            ...
            if dry_run:
                ...
                if result.returncode == 0:
                    ...
                    click.echo("  ⚠️  Предупреждение: ошибка создания бэкапа")
        ...
        files_to_sync = [
            'app', 'public', 'xml', '*.py', '*.toml', 'requirements.txt', 'README.md'
        ]
        ...
        rsync_cmd = ['rsync', '-rltz'] + exclude_patterns + files_to_sync + [f"{server}:{app_folder}"]
        ...
        click.echo(f"🎉 Деплой завершен успешно!")
        
        if not install:
            click.echo("💡 Для установки плагина выполните:")
            click.echo(f"   ssh {server} 'cd {app_folder} && source venv/bin/activate && sudo billmgr-addon deploy install --plugin-name {plugin_name}'")
