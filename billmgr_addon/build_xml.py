    ...
    def execute_import(self, xml_src_path):
        ...
        for import_element in import_elements:
            ...
            if import_path is None:
                sys.exit(f'Attribute "path" is not found in {import_element_string}\nFailed to import in {self.path}')

            if import_as is not None and import_as == '':
                sys.exit(f'Attribute "as" in {import_element_string} can not be empty string\nFailed to import in {self.path}')
            ...
            if import_path.is_absolute():
                sys.exit(f'{import_element_string} path must be relative\nFailed to import in {self.path}')
            ...
            try:
                ...
                print(e)
                sys.exit(f'{import_element_string} must be valid file path inside xml_src_path\nFailed to import in {self.path}')

            if self.has_parent_path(import_file_path):
                sys.exit(f'{import_element_string} circular reference in xml_src_path\nFailed to import in {self.path}')
