...
class XmlEntry:
    ...
    def __init__(self, absolute_path: Path, root_element: Element, parent_entry: Optional['XmlEntry'] = None):
        """
    ...
    def execute_import(self) -> None:
        ...
        xml_src_path = self.path.parent.parent if self.path.parent.name == 'src' else self.path.parent
        ...
        for import_element in import_elements:
            ...
            if import_path is None:
                raise ValueError(f'Attribute "path" is not found in {import_element_string}\nFailed to import in {self.path}')

            if import_as is not None and import_as == '':
                raise ValueError(f'Attribute "as" in {import_element_string} can not be empty string\nFailed to import in {self.path}')
            ...
            if import_path.is_absolute():
                raise ValueError(f'{import_element_string} path must be relative\nFailed to import in {self.path}')
            ...
            try:
                ...
                is_relative = xml_src_path == import_file_path or xml_src_path in import_file_path.parents 
                if not is_relative:
                    raise FileNotFoundError()
                    
            except Exception as e:
                raise ValueError(f'{import_element_string} must be valid file path inside xml_src_path\nFailed to import in {self.path}')

            # Проверяем циклические ссылки
            if self.has_parent_path(import_file_path):
                raise ValueError(f'{import_element_string} circular reference in xml_src_path\nFailed to import in {self.path}')
